...
//Jeśli klikneliśmy Modyfikuj to pojawił sie kolejny formularz i tutaj jest jego obsługa czyli aktualizacja danych
// użytkownika
if (isset($_POST['submit'])) {

//    Sprawdzenie czy ktoś wszedł tutaj dopiero po wciśnięciu przycisku z formularza
    buttonChecker($_POST['submit'], 'listUser.php');
...
//    Przez to użytkownik nie musi zmieniać emaila jak wejdzie w edycje profilu
    if ($sameEmail === $email) {
        $validateUpdate = true;
    }
    ...
    $editAccount = new SignupContr($uid, $name, $surename, $pwd, $pwdRepeat, $email, $role, $pesel123);

//Walidacja danych i aktualizacja informacji o użytkowniku
    $editAccount->updateUser($errors, $validateUpdate);
...
//    Jeśli użytkownik nadusił przycisk Reset Hasła to tutaj jest obsługa formularza z nim związanego
} elseif (isset($_POST['change'])) {
    //    Sprawdzenie czy ktoś wszedł tutaj dopiero po wciśnięciu przycisku z formularza
    buttonChecker($_POST['change'], 'listUser.php');
...
//Obsługa formularza związanego z usuwaniem konta
...
//    Wyświetlenie formularza po kliknięciu Modyfikuj
} else {
    if (isset($_POST['modify'])) $uid = $_POST['modify'];
    elseif (isset($_POST['passw'])) $uid = $_POST['passw'];
    else {
        //    Sprawdzenie czy ktoś wszedł tutaj dopiero po wciśnięciu przycisku z formularza
        buttonChecker($_POST['delete'], 'listUser.php');
                    ...
                    <div class="card-body p-4 p-sm-5">
                        ...
                        <form method="POST" enctype="multipart/form-data" action="">
                            ...
                            <div class="d-grid mb-2">
                                <button class="btn btn-lg btn-primary btn-login fw-bold text-uppercase" name="change">
                                    Zmień hasło
                                </button>
